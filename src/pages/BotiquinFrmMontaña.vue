          ...
          <q-card class="q-pa-md">
            <q-card-section>
              ...
              <div v-if="modoEdicion && inventarioEditando" class="q-mb-md">
                <q-banner class="bg-purple-1 text-purple-8 rounded-borders">
                  <template v-slot:avatar>
                    <q-icon name="edit" color="purple" size="md" />
                  </template>
                  <div class="text-weight-medium">Editando Botiquín de Montaña</div>
              ...
              <div class="text-h6 q-mb-md">
                {{ modoEdicion ? 'Actualizar Items del Botiquín' : 'Agregar Items del Botiquín' }}
              </div>

              <!-- Selector de item -->
              <q-select
                v-model="itemSeleccionado"
                :options="itemsDisponibles.montaña"
                option-label="nombre"
...
onMounted(async () => {
  console.log('🔍 Iniciando carga de página BotiquinFrmMontaña...')
  ...
  try {
    ...
    console.log('✅ Items de montaña cargados:', itemsDisponibles.montaña)
  } catch (error) {
  ...
  try {
    console.log('📋 Cargando historial de inventarios de montaña...')
    const historial = await cargarHistorialInventarios('montaña')
...
const agregarItem = () => {
  ...
  const nuevoItem = {
    ...
    tipo_kit: 'montania', // Agregar tipo para actualización (montania para BD)
  }
...
const registrarBotiquin = async () => {
  ...
  try {
    ...
    if (modoEdicion.value) {
      ...
      console.log('Iniciando actualización desde formulario MONTAÑA:', {
      ...
      $q.notify({
        type: 'positive',
        message: 'Botiquín de montaña actualizado exitosamente',
      })
...
const irACompras = async () => {
  ...
  try {
    ...
    await crearOrdenCompra(itemsAgregados.value, 'montania')
