<template>
  <q-layout view="lHh Lpr lFf">
    <q-header elevated>
      <q-toolbar>
        <q-toolbar-title class="text-white">Formulario Botiquín - Escolar</q-toolbar-title>
      </q-toolbar>
    </q-header>
      ...
      <q-form @submit.prevent="onSubmit" class="q-gutter-md form-grid">
        <div>
          <div class="grid-container">
            <div v-for="(item, idx) in escolarItems" :key="idx" class="grid-item">
              <q-input
...
const escolarItems = [
  'Termómetro digital',
  'Gasas estériles (4x4)',
  'Vendas elásticas medianas',
  'Apósitos adhesivos (curitas)',
  'Cinta médica (micropore)',
  'Alcohol de 70%',
  'Solución salina estéril',
  'Guantes descartables (mínimo 3 pares)',
  'Tijeras punta roma',
  'Lista de contactos de emergencia médica',
  'Registro de atención (nombre, fecha, motivo)',
  'Medicamentos autorizados por apoderado (solo con permiso por escrito)',
]

const cantidades = reactive({})
escolarItems.forEach((item) => (cantidades[item] = 0))

function onSubmit() {
  let hist = []
  const savedHist = localStorage.getItem('historialBotiquinEscolar')
  if (savedHist) {
  ...
  localStorage.setItem('historialBotiquinEscolar', JSON.stringify(hist))
  historial.value = hist
  alert(
    'Guardado para Escolar: ' +
