<template>
  <q-layout view="lHh Lpr lFf">
    <q-header elevated>
      <q-toolbar>
        <q-toolbar-title class="text-white">Formulario Botiquín - Industrias</q-toolbar-title>
      </q-toolbar>
    </q-header>
...
const industriaItems = [
  ...
  'Férulas rígidas (para brazos y piernas)',
  'Collarín cervical blando',
  'Vendas elásticas (medianas y grandes)',
  'Vendas triangulares',
  'Gasas estériles (4x4 y 10x10)',
  'Apósitos grandes (para hemorragias)',
  'Cinta médica resistente al sudor',
  'Tijeras de trauma (punta roma, corta ropa)',
  'Suero fisiológico estéril (botella de 500 ml y ampollas)',
  'Alcohol 70%',
  'Clorhexidina o yodopovidona',
  'Guantes reforzados (nitrilo o vinilo)',
  'Mascarillas de protección respiratoria (N95 o similar)',
  'Compresas frías de activación instantánea',
  'Oxímetro de pulso portátil',
  'Registro de accidentes (formato impreso)',
  'Manual de atención en trauma industrial',
]
...
function onSubmit() {
  let hist = []
  const savedHist = localStorage.getItem('historialBotiquinIndustrias')
  if (savedHist) {
  ...
  localStorage.setItem('historialBotiquinIndustrias', JSON.stringify(hist))
  historial.value = hist
  alert(
    'Guardado para Industrias: ' +
